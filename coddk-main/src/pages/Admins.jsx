...
export default function Admins() {
  ...
  const fetchAdmins = async () => {
    try { const res = await API.get("/employees?role=admin"); setAdmins(res.data); } catch { /* silent */ }
  };
  ...
  const handleAdd = async (e) => {
    ...
    try {
      await API.post("/employees", { ...form, role: "admin", directAdd: form.directAdd || false });
      setForm({ name: "", username: "", password: "" });
      setMessage("✅ تم إضافة المدير بنجاح.");
      fetchAdmins();
    } catch (err) { setError(err.response?.data?.msg || "تعذّر الإضافة."); }
    finally { setLoading(false); }
  };

  const handleDelete = async (id) => {
    try { await API.delete(`/employees/${id}`); setDeleteConfirm(null); fetchAdmins(); }
    catch { setError("تعذّر حذف المدير."); }
  };

  const openEdit = (admin) => {
    setEditMember(admin);
    setEditFields({ name: admin.name, username: admin.username, password: "", directAdd: admin.directAdd || false });
    setEditErr("");
    setEditOpen(true);
  };
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      setEditOpen(false); fetchAdmins();
    } catch (err) { setEditErr(err.response?.data?.msg || "فشل التعديل."); }
    ...
    <div style={{ direction: "rtl", minHeight: "calc(100vh - 64px)", background: "var(--dark)" }}>
      <div style={{ background: "var(--surface)", borderBottom: "1px solid var(--border)", padding: "0 16px" }}>
        <div className="container" style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 4, overflowX: "auto", maxWidth: "100%" }}>
          ...
          <Link to="/admins" style={{ padding: "14px 16px", color: "var(--primary)", fontWeight: 700, textDecoration: "none", borderBottom: "2px solid var(--primary)", fontSize: "0.9rem", whiteSpace: "nowrap" }}>🛡️ المدراء</Link>
      ...
      <div className="container" style={{ padding: "24px 16px" }}>
        <h2 style={{ color: "#fff", fontWeight: 800, marginBottom: 6, fontSize: "1.4rem" }}>إدارة المدراء</h2>
        <p style={{ color: "var(--text-muted)", marginBottom: 24, fontSize: "0.875rem" }}>إضافة وإدارة حسابات المدراء (مديرين النظام).</p>
        ...
        <div className="row g-4">
          <div className="col-12 col-lg-4">
            <div className="glass-card" style={{ padding: "20px" }}>
              <h5 style={{ color: "#fff", fontWeight: 700, marginBottom: 18, fontSize: "1rem" }}>إضافة مدير جديد</h5>
              <form onSubmit={handleAdd}>
                <div style={{ marginBottom: 12 }}>
                  <label style={{ color: "var(--text-muted)", fontSize: "0.8rem", fontWeight: 600, display: "block", marginBottom: 6 }}>الاسم الكامل</label>
                  <input style={inputStyle} type="text" placeholder="اسم المدير" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
                </div>
                <div style={{ marginBottom: 12 }}>
                  <label style={{ color: "var(--text-muted)", fontSize: "0.8rem", fontWeight: 600, display: "block", marginBottom: 6 }}>اسم المستخدم</label>
                  <input style={{ ...inputStyle, direction: "ltr", textAlign: "left" }} type="text" placeholder="admin.username" value={form.username} onChange={e => setForm({ ...form, username: e.target.value })} />
                </div>
                <div style={{ marginBottom: 18 }}>
                  ...
                  <div style={{ position: "relative" }}>
                    ...
                    <button type="button" onClick={() => setShowPass(!showPass)} style={{
                      position: "absolute", left: 12, top: "35%", transform: "translateY(-50%)",
                    ...
                    {user?.role === "superadmin" && (
                      <div style={{ marginBottom: 18, display: "flex", alignItems: "center", gap: 10 }}>
                        <input
                          type="checkbox"
                          id="directAddNewAdmin"
                          checked={form.directAdd || false}
                          onChange={(e) => setForm({ ...form, directAdd: e.target.checked })}
                          style={{ width: 18, height: 18 }}
                        />
                        <label htmlFor="directAddNewAdmin" style={{ color: "var(--text-muted)", fontSize: "0.875rem", margin: 0 }}>
                ...
                <button type="submit" className="btn btn-primary w-100" disabled={loading}>
                  {loading ? <><span className="spinner-border spinner-border-sm" style={{ marginLeft: 6 }} />جارٍ الإضافة...</> : "إضافة مدير"}
                </button>
              </form>
            </div>
          </div>
          ...
          <div className="col-12 col-lg-8">
            <div className="glass-card" style={{ overflow: "hidden" }}>
              <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border)" }}>
                <h5 style={{ color: "#fff", fontWeight: 700, margin: 0, fontSize: "1rem" }}>المدراء ({admins.length})</h5>
              </div>
              {admins.length === 0 ? (
                <div style={{ textAlign: "center", padding: "40px", color: "var(--text-muted)" }}>
                  <div style={{ fontSize: "2.5rem", marginBottom: 12 }}>🛡️</div>
                  <p>لا يوجد مدراء حتى الآن.</p>
                </div>
              ) : (
                    ...
                    <tbody>
                      {admins.map((admin, idx) => (
                        <tr key={admin._id}>
                          ...
                          <td style={{ direction: "ltr", fontFamily: "monospace", color: "var(--text-muted)", fontSize: "0.85rem" }}>{admin.username}</td>
                          <td>
                            <div style={{ display: "flex", gap: 6 }}>
                              <button className="btn btn-outline-warning btn-sm" onClick={() => openEdit(admin)} style={{ fontSize: "0.75rem" }}>✏️</button>
                              <button className="btn btn-outline-danger btn-sm" onClick={() => setDeleteConfirm(admin._id)} style={{ fontSize: "0.75rem" }}>🗑️</button>
                            </div>
                          </td>
                        </tr>
                      ))}
      ...
      <Modal open={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} title="⚠️ تأكيد الحذف"
        footer={<><button className="btn btn-danger" onClick={() => handleDelete(deleteConfirm)}>حذف المدير</button><button className="btn btn-outline-primary" onClick={() => setDeleteConfirm(null)}>إلغاء</button></>}>
        <p style={{ color: "var(--text-muted)", lineHeight: 1.7, margin: 0 }}>هل أنت متأكد من إزالة هذا المدير؟</p>
      </Modal>

      <Modal open={editOpen && !!editMember} onClose={() => setEditOpen(false)} title="✏️ تعديل بيانات المدير"
        footer={<>
        ...
        {user?.role === "superadmin" && (
          <div className="mb-3" style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <input
              ...
              style={{ width: 18, height: 18 }}
            />
            <label htmlFor="directAddAdmin" style={{ color: "var(--text-muted)", fontSize: "0.875rem", margin: 0 }}>
              السماح بإضافة الطلاب مباشرة (بدون تحقق)
            </label>
          </div>
        )}
